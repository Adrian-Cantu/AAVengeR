...
AVVenger was run on Aradhana's sequences (210426\_M03249\_0176\_000000000-G7P6T) using a database that includes the dog genome, the plasmid backbone, and the Heavy and light chain cassettes (dog\_plus\_transgene.2bit). Options used were ```sequence.qualTrim.code: '+' , alignment.removeVectorReadPairs: FALSE```. All sites with Sonic abundance less than 1 were excluded from the analysis.
...
get_insertion_type <- function(x) {
  ...
  x <- ifelse(startsWith(xx, 'chr'),'dog',
    ifelse(startsWith(xx, 'PAAVc8_plasmid_backbone'),"plasmid_BB",
      ifelse(startsWith(xx, 'LightChain_transgene'),'LightChain','HeavyChain')
    )
  )
  return(x)
}

all_sites<- readRDS('dog_plus_plasmid_transgene/out1/sites.rds')
...
Relative abundance of dog insertion sites in the dog samples ranges from 48\% to 58\%
...
print(print_table, comment=F,include.rownames = FALSE,
      hline.after = c(-1,0,2,4,8),floating=FALSE)
...
dog_mesup <- all_sites_summ %>% filter(sample=="GTSP4181") %>% 
  transmute(sample=sample, replicate=replicate,
    messupness= sqrt((dog_relAbund-1)^2+plasmid_BB_relAbund^2+LC_relAbund^2+HC_relAbund^2))
...
# Position of insertions into the plasmid backbone
