---
title: "Plasmid_and_cassettes_insertions_report"
output: pdf_document
...
AVVenger was run on Aradhana's sequences (210426\_M03249\_0176\_000000000-G7P6T) using a database that includes the dog genome, the plasmid backbone, and the Heavy and light chain cassettes (dog\_plus\_plasmid_parts.2bit). Options used were ```sequence.qualTrim.code: '+' , alignment.removeVectorReadPairs: FALSE```.
...
print(print_table, comment=F,include.rownames = FALSE,
      hline.after = c(-1,0,4,7,11),floating=FALSE)
...
sites_l_table <- all_sites[!startsWith(all_sites$seqnames, 'chr'),] %>% 
...
sites_l_table[sites_l_table$insertion_type=="HeavyChain",] %>% mutate(insertion_type = NULL) %>%
...
sites_l_table[sites_l_table$insertion_type=="LightChain",] %>% mutate(insertion_type = NULL) %>%
...
dog_sites_l_table_tmp <- all_sites[startsWith(all_sites$seqnames, 'chr'),]
